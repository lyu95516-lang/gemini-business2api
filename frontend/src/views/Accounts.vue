        ...
        <div class="scrollbar-slim flex-1 overflow-y-auto px-6 py-4">
          ...
          <div v-else class="space-y-4">
            ...
            <div class="rounded-2xl border border-border bg-muted/30 px-3 py-2 text-xs text-muted-foreground">
              ...
              <p class="mt-1 font-mono">cfworker----email----jwtToken</p>
...
const parseImportLines = (raw: string) => {
  ...
  lines.forEach((line, index) => {
    ...
    if (parts[0].toLowerCase() === 'cfworker') {
      if (parts.length < 3 || !parts[1] || !parts[2]) {
        errors.push(`第 ${lineNo} 行格式错误（cfworker 需要 email 和 jwt_token）`)
        return
      }
      const email = parts[1]
      ...
      items.push({
        ...
        mail_base_url: parts[3] || undefined,
        mail_domain: parts[4] || undefined,
      })
      return
    }
